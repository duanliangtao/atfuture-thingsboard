...
public class DeviceController extends BaseController {
     ...
     */
    @PreAuthorize("hasAnyAuthority('TENANT_ADMIN','CUSTOMER_USER')")
    @RequestMapping(value = "/device/{deviceId}/active",method = RequestMethod.GET)
    @ResponseBody
    public DeviceActiveResult activeDevice(@PathVariable(DEVICE_ID) String strDeviceId) throws ThingsboardException {
        checkParameter(DEVICE_ID, strDeviceId);
        try {
            DeviceId deviceId = new DeviceId(toUUID(strDeviceId));
            Device device = checkDeviceId(deviceId, Operation.READ_CREDENTIALS);
            ...
            DeviceActiveResult rs = deviceActiveService.activeDeviceByRest(device, deviceCredentials);

            logEntityAction(deviceId, device,
                    device.getCustomerId(),
                    ActionType.DEVICE_ACTIVE, null, strDeviceId);
            return rs;
        } catch (Exception e) {
            logEntityAction(emptyId(EntityType.DEVICE), null,
                    null,
                    ActionType.DEVICE_ACTIVE, e, strDeviceId);
            throw handleException(e);
        }
    }
